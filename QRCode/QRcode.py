...
def do_QRcode(fname):
    ...
    while 0 != len(a_line):
        ...
        if (3 > len(a_split)):
            sys.stdout.write("  $$$ ERROR $$$: instruction must have 3 tab-separated fields, not %d<br><br>\n\n" % len(a_split))
            line_good = False
        elif 4 != (len(a_split[0]) - a_split[0].find(".png")):
            sys.stdout.write("  $$$ ERROR $$$: QR filename must be *.png not '%s'<br><br>\n\n" % a_split[0])
            line_good = False
        elif (-1 != a_split[0].find("\\")) or (-1 != a_split[0].find("/")):
            sys.stdout.write("  $$$ ERROR $$$: QR filename not change directories as in '%s'<br><br>\n\n" % a_split[0])
            line_good = False
